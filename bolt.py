...
def make_thumbnail(rgb, height=THUMB_HEIGHT):
    """Downscale an RGB ndarray to ~`height`px tall and return JPEG bytes."""
...
def save_bolt_thumb(rgb, dir_, frame):
    os.makedirs(dir_, exist_ok=True)
    with open(os.path.join(dir_, f"{frame}.jpg"), "wb") as f:
        f.write(make_thumbnail(rgb))
...
def refine_event(video, start_frame, end_frame, peak_frame, window=4, thumb_dir=None):
    """Find the best channel frame within a flash window. Returns BoltResult.

    With thumb_dir set, saves a color thumbnail of the chosen frame for each
    real bolt (the RGB is already decoded here, so it is near-free).
    """
    lo, hi = max(0, start_frame - window), end_frame + window
    win = {idx: rgb for idx, rgb in video.iter_range(lo, hi, downscale_height=ANALYZE_HEIGHT)}
    if not win:
        return BoltResult(peak_frame, peak_frame, 0.0, 0, False)
    grays = {idx: to_gray(rgb) for idx, rgb in win.items()}
    ...
    is_bolt = best_h >= BOLT_MIN_VERTICAL
    if is_bolt and thumb_dir is not None and best_idx in win:
        save_bolt_thumb(win[best_idx], thumb_dir, best_idx)
    return BoltResult(peak_frame, best_idx, best_score, best_h, is_bolt)


def refine_events(video, events, window=4, progress=None, thumb_dir=None):
    ...
    for i, e in enumerate(events):
        results.append(refine_event(video, e.start_frame, e.end_frame,
                                     e.peak_frame, window=window, thumb_dir=thumb_dir))
        if progress:
            progress(i + 1, total)
    return results
...
def _bolt_cache_path(path, params):
    ...
    return os.path.join(folder, f".lf_bolt_{_bolt_key(path, params)}.json")


def thumbs_dir(path, params):
    folder = os.path.dirname(os.path.abspath(path)) or "."
    return os.path.join(folder, f".lf_thumbs_{_bolt_key(path, params)}")


def thumb_path(path, params, frame):
    return os.path.join(thumbs_dir(path, params), f"{frame}.jpg")
